  ...
  constructor(dispatch) {
    ...
    if (dispatch.proxyAuthor !== "caali") {
      ...
      if (options) {
        ...
        if (settingsVersion) {
          dispatch.settings = require("./" +
            (options.settingsMigrator || "settings_migrator.js"))(
            dispatch.settings._version,
            settingsVersion,
            dispatch.settings
          );
    ...
    function class_position_check(class_position) {
      ...
      switch (class_position) {
        ...
        default: {
          debug_message(
            true,
            "Failed to find class_position value:",
            class_position
          );
        }
      }
    ...
    function handle_event(
      ...
      debug_message(
        d,
        `${called_from_identifier}: ${id} | Started by: ${unique_id} | key: ${
          key + stage_string
        }`
      );
      if (stage !== false) {
    ...
    function start_events(events = [], ent, speed = 1.0) {
      ...
      for (let event of events) {
        ...
        if (!func)
          debug_message(true, "An event has invalid type:", event["type"]);
        ...
        if (spguide) {
          if (ent)
            return handle_event(
              ...
              debug.debug ||
                debug.skill ||
                (ent["templateId"] % 1 === 0 ? debug.boss : false),
              e.speed,
              e.stage
            );
        } else if (esguide) {
    ...
    dispatch.hook("S_BOSS_GAGE_INFO", 3, (e) => {
      ...
      if (dispatch.settings.enabled && guide_found) {
        ...
        if (ent)
          return handle_event(
            ent,
            Math.floor((Number(e.curHp) / Number(e.maxHp)) * 100),
            "Health",
            "h",
            debug.debug || debug.hp
          );
      }
    });
    ...
    command.add(["guide"], {
      ...
      debug(arg1) {
        if (!arg1) {
          ...
          for (let [key, value] of Object.entries(debug)) {
            command.message(
              `debug(${key}): ${value ? "enabled" : "disabled"}.`
            );
          }
          return;
        ...
        command.message(
          `Guide module debug(${arg1}) mode has been ${
            debug[arg1] ? "enabled" : "disabled"
          }.`
        );
      },
      // Testing events
      event(arg1, arg2) {
        ...
        if (arg1 === "load") {
          if (!arg2)
            return command.message(
              `Invalid values for sub command "event" ${arg1}`
            );
        ...
        if (
          arg1 === "trigger"
            ? !active_guide[arg2]
            : !arg1 || !function_event_handlers[arg1] || !arg2
        )
          return command.message(
            `Invalid values for sub command "event" ${arg1} | ${arg2}`
          );
      ...
      voice() {
        ...
        text_handler({
          sub_type: "PRMSG",
          message_RU: `Голосовое сообщение ${
            dispatch.settings.speaks ? "Вкл" : "Выкл"
          }.`,
          message: `Text-to-speech ${dispatch.settings.speaks ? "on" : "off"}.`,
        });
      },
      stream() {
        sendtoParty = !sendtoParty;
        command.message("sendtoParty is now: " + sendtoParty);
      ...
      spawnObject(arg1) {
        ...
        if (arg1) {
          ...
          if (sd_id) {
            dispatch.settings.dungeons[sd_id].spawnObject = !dispatch.settings
              .dungeons[sd_id].spawnObject;
            text_handler({
              sub_type: "PRMSG",
              message_RU: `Спавн объектов для данжа ${
                dispatch.settings.dungeons[sd_id].name_RU
              } [${dispatch.settings.dungeons[sd_id].id}]: ${
                dispatch.settings.dungeons[sd_id].spawnObject ? "Вкл" : "Выкл"
              }.`,
              message: `Spawning objects for dungeon ${
                dispatch.settings.dungeons[sd_id].name_RU
              } [${dispatch.settings.dungeons[sd_id].id}] has been ${
                dispatch.settings.dungeons[sd_id].spawnObject ? "on" : "off"
              }.`,
            });
          } else {
            text_handler({
          ...
          text_handler({
            sub_type: "PRMSG",
            message_RU: `Спавн объектов: ${
              dispatch.settings.spawnObject ? "Вкл" : "Выкл"
            }.`,
            message: `Spawn objects ${
              dispatch.settings.spawnObject ? "on" : "off"
            }.`,
          });
        }
      },
      verbose(arg1) {
        ...
        if (arg1) {
          sd_id = find_dungeon_index(arg1);
          if (sd_id) {
            dispatch.settings.dungeons[sd_id].verbose = !dispatch.settings
              .dungeons[sd_id].verbose;
            text_handler({
              sub_type: "PRMSG",
              message_RU: `Показ сообщений для данжа ${
                dispatch.settings.dungeons[sd_id].name_RU
              } [${dispatch.settings.dungeons[sd_id].id}]: ${
                dispatch.settings.dungeons[sd_id].verbose ? "Вкл" : "Выкл"
              }.`,
              message: `Messaging for dungeon ${
                dispatch.settings.dungeons[sd_id].name_RU
              } [${dispatch.settings.dungeons[sd_id].id}] has been ${
                dispatch.settings.dungeons[sd_id].verbose ? "on" : "off"
              }.`,
            });
          } else {
            text_handler({
      ...
      alert() {
        ...
        text_handler({
          sub_type: "PRMSG",
          message_RU: `Сообщения в чат: ${
            dispatch.settings.notice ? "Вкл" : "Выкл"
          }.`,
          message: `Virtual captain has been ${
            dispatch.settings.notice ? "on" : "off"
          }.`,
        });
      },
      systemNotice() {
        ...
        command.message(
          `system Notice ${dispatch.settings.systemNotice ? "on" : "off"}.`
        );
        text_handler({
          sub_type: "PRMSG",
          message_RU: `сообщения в группе: ${
            dispatch.settings.systemNotice ? "Вкл" : "Выкл"
          }.`,
          message: `system Notice ${
            dispatch.settings.systemNotice ? "on" : "off"
          }.`,
        });
      },
      cr() {
      ...
      help() {
        text_handler({
          ...
          message:
            "guide,  on/off, default system notification, notification color yellow",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU: "guide voice, вкл./выкл. голосовые сообщения",
          message: "guide voice, text-to-speech on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU: "guide systemNotice, вкл./выкл. сообщений в группе",
          message: "guide systemNotice, system Notice on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU: "guide alert, вкл./выкл. сообщений в чате",
          message: "guide alert, Virtual captain  notifie on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU: "guide spawnObject, вкл./выкл. спавна объектов",
          message: "guide spawnObject, spawn Object on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU:
            "guide stream, вкл./выкл. режима стрима (скрытие сообщений)",
          message: "guide stream,(stream)on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU:
        ...
        text_handler({
          ...
          message: "verbose id, messaging for dungeon on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU:
            "guide spawnObject id, вкл./выкл. спавна объектов для данжа, где id - идентификатор данжа",
          message: "guide spawnObject id, spawn objects for dungeon on/off",
        });
        text_handler({
          sub_type: "PRMSG",
          message_RU: "guide cc, отобразить текущий цвет системного сообщения",
          message:
            "guide cc,View the current system message notification color",
        });
        text_handler({
          sub_type: "CRMSG",
    ...
    function spawn_handler(event, ent, speed = 1.0) {
      ...
      if (!event["sub_delay"])
        return debug_message(true, "Spawn handler needs a sub_delay");
      ...
      switch (sub_type) {
        ...
        default: {
          return debug_message(
            true,
            "Invalid sub_type for spawn handler:",
            event["sub_type"]
          );
        }
      }
    ...
    function despawn_handler(event) {
      ...
      switch (sub_type) {
        ...
        default:
          return debug_message(
            true,
            "Invalid sub_type for despawn handler:",
            event["sub_type"]
          );
      }
    }
    ...
    function text_handler(event, ent, speed = 1.0) {
      // Fetch the message
      const message =
        event[`message_${language}`] ||
        event[`message_${language.toUpperCase()}`] ||
        event["message"];
      // Make sure sub_type is defined
      if (!event["sub_type"])
        return debug_message(true, "Text handler needs a sub_type");
      ...
      switch (event["sub_type"]) {
        ...
        default: {
          return debug_message(
            true,
            "Invalid sub_type for text handler:",
            event["sub_type"]
          );
        }
      }
      ...
      timers[event["id"] || random_timer_id--] = setTimeout(() => {
        switch (event["sub_type"]) {
          case "notification":
            return dispatch.toClient("S_CHAT", 3, sending_event);
          case "alert":
            return dispatch.toClient("S_CHAT", 3, sending_event);
        }
      }, (event["delay"] || 0) / speed);
    ...
    function sendMessage(message) {
      ...
      // Team leader notice
      else if (dispatch.settings.notice) {
    ...
    function func_handler(event, ent, speed = 1.0) {
      ...
      if (!event["func"])
        return debug_message(true, "Func handler needs a func");
