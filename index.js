  ...
  constructor(dispatch) {
      ...
      if (enabled && guide_found) {
        ...
        if (ent)
          return handle_event(
            ...
            debug.debug ||
              debug.skill ||
              (ent["templateId"] % 1000 === 0 ? debug.boss : false),
            e.speed,
            e.stage
          );
      }
    ...
    command.add("guide", {
      ...
      event(arg1, arg2) {
        ...
        if (arg1 === "trigger") start_events(active_guide[arg2], player);
        // Call a function handler with the event we got from arg2 with yourself as the entity
        else function_event_handlers[arg1](JSON.parse(arg2), player);
      },
      stream() {
        stream = !stream;
        command.message(
          `Streamer mode has been ${stream ? "enabled" : "disabled"}.`
        );
      },
      $default() {
        enabled = !enabled;
        command.message(
          `Guide module has been ${enabled ? "enabled" : "disabled"}.`
        );
      },
    });
    ...
    function text_handler(event, ent, speed = 1.0) {
      ...
      const message =
        event[`message_${dispatch.language}`] ||
        event[`message_${dispatch.language.toUpperCase()}`] ||
        event["message"];
      ...
      timers[event["id"] || random_timer_id--] = setTimeout(() => {
        if (!stream) {
          ...
          return dispatch.toClient("S_CHAT", 3, {
            channel: 1,
